...
const UserTaskCrud = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('https://backend-piii-24-main-one.vercel.app/api/user');
  ...
  const updateUser = async (id, userData) => {
    try {
      const response = await fetch(`https://backend-piii-24-main-one.vercel.app/api/user/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const deleteUser = async (id) => {
    try {
      await fetch(`https://backend-piii-24-main-one.vercel.app/api/user/${id}`, {
        method: 'DELETE',
      });
      fetchUsers();
    } catch (error) {
